...
For warehouse `wh-001`: three shift templates — "Morning Shift" 06:00–14:00, "Evening Shift" 14:00–22:00, "Night Shift" 22:00–06:00 next day (to exercise the midnight-wrap case) — all `is_active = true` (pre-activated in seed data so Sprint 3+ demos aren't blocked). The three-shift pattern was folded in from a manual DB Browser edit per [DECISION_LOG.md](../DECISION_LOG.md) 2026-09-10, replacing the original two-shift (9-hour Morning) seed. One `LABOR_REGULATION` (US Fair Labor Standards Act, `max_hours_per_day=8`, `max_hours_per_week=40`, `break_interval_mins=60`, `region="USA"`), also seeded `is_active = true`, attached to all three shift templates via `REGULATION_SHIFT`. Each shift is now 8 hours gross (480 min); with the 60-minute break, `available_shift_minutes = 420` for all three — this is an accepted, documented deviation from the ERD reference §4 worked example (which used a 9-hour/540-minute shift to reach 480), not a bug.
